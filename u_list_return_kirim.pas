...
uses
  ...
  ExtCtrls, sPanel, sSkinProvider,U_fungsi, sSpeedButton, Mask, sMaskEdit,
...
uses u_dm, u_utama, u_return_kirim;
{$R *.dfm}
...
btnSimpan.Enabled:= False;
  ...
  begin
    fungsi.SQLExecT(dm.Q_list_return_kirim,'call sp_belum_return_kirim("'+cbCabang.Text+'")',True);
    btnSimpan.Enabled:= True;
  end;
end;


procedure Tf_list_return_kirim.FormClose(Sender: TObject;
...
procedure Tf_list_return_kirim.t_dataCellDblClick(
...
if SelectDirectory('Pilih Folder', ExtractFileDrive(dm.WPath), tempat) then
  begin
    ...
    begin
      ...
      nama_file:= tempat+'\'+dm.Q_list_return_kirim.fieldbyname('kd_return_kirim').AsString+'.srk';
      ...
      Writeln(F, formatdatetime('dd/MM/yyyy',dm.Q_list_return_kirim.fieldbyname('tgl_return_kirim').AsDateTime));
      ...
      begin
        ...
        begin
          qty:= dm.Q_temp.FieldByName('qty_return_kirim').AsFloat;
