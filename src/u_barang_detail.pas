...
uses
  ...
  Buttons, sSpeedButton, sCurrEdit, sCurrencyEdit, sButton, sCheckBox,
...
begin
  ...
  begin
    ...
    if ed_bar2.Color = clred then
      ed_bar2.SetFocus;
    if ed_bar1.Color = clred then
      ed_bar1.SetFocus;
    exit;
  end;
  ...
  begin
    ...
    if ed_bar3.Color = clblue then
      ed_bar3.SetFocus;
    if ed_bar2.Color = clblue then
      ed_bar2.SetFocus;
    if ed_bar1.Color = clblue then
      ed_bar1.SetFocus;
    exit;
  end;
    ...
    begin
      ...
      begin
        ...
        fungsi.SQLExec(dm.Q_Exe,
          'INSERT IGNORE INTO tb_barang_supp(kd_perusahaan,kd_suplier,kd_barang,`update`) values ("' +
          cabang[i] + '","' + dm.kd_perusahaan + '","' + ed_pid.Text +
          '",date(now()))', False);
      end;
    end;
...
procedure TF_barang_det.SimpanDatabase(perusahaan: string; b_aktif: string;
  status_simpan: Boolean);
begin
  ...
  begin
    //simpan data
    fungsi.SQLExec(dm.Q_Exe,
      'insert into tb_barang (kd_perusahaan,kd_barang,n_barang,kd_jenis,kd_kategori, ' +
      'kd_golbrg,kd_merk,kd_sat1,kd_sat2,kd_sat3,barcode1,barcode2,barcode3,Qty1,Qty2,minstok,maxstok, ' +
      'leadtime,aktif,minor,`update`)values ("' + perusahaan + '","' + ed_pid.Text
      + '","' + ed_nama.Text + '","' + kd_jenis + '","' + kd_tag + '","' +
      kd_gol + '","' + kd_merk + '","' + kd_sat[1] + '","' + kd_sat[2] + '","' +
      kd_sat[3] + '","' + ed_bar1.Text + '","' + ed_bar2.Text + '","' + ed_bar3.Text
      + '","' + ed_qty1.Text + '","' + ed_qty2.Text + '","' + ed_minstok.Text +
      '","' + ed_maxstok.Text + '","' + ed_time.Text + '","' + b_aktif + '","' +
      ed_minor.Text + '",CURRENT_TIMESTAMP)', false);
  end
  else
  begin
    //ubah data
    fungsi.SQLExec(dm.Q_Exe, 'update tb_barang set n_barang="' + ed_nama.Text +
      '",kd_jenis="' + kd_jenis + '",kd_kategori="' + kd_tag + '",kd_golbrg="' +
      kd_gol + '",kd_merk="' + kd_merk + '",kd_sat1="' + kd_sat[1] +
      '",kd_sat2="' + kd_sat[2] + '",kd_sat3="' + kd_sat[3] + '",barcode1="' +
      ed_bar1.Text + '",barcode2="' + ed_bar2.Text + '",barcode3="' + ed_bar3.Text
      + '",Qty1="' + ed_qty1.Text + '",Qty2="' + ed_qty2.Text + '",minstok="' +
      ed_minstok.Text + '",maxstok="' + ed_maxstok.Text + '",leadtime="' +
      ed_time.Text + '",aktif="' + b_aktif + '",minor="' + ed_minor.Text +
      '",`update`=CURRENT_TIMESTAMP where kd_perusahaan="' + perusahaan +
      '" and kd_barang="' + ed_pid.Text + '"', false);
  end;
end;

procedure TF_barang_det.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
begin
  ...
  if (ed_bar1.Text <> ed_bar2.Text) and (ed_bar1.Text <> ed_bar3.Text) then
    if ed_bar1.Color <> clblue then
      ed_bar1.Color := clwhite;
  if (ed_bar2.Text <> ed_bar1.Text) and (ed_bar2.Text <> ed_bar3.Text) then
    if ed_bar2.Color <> clblue then
      ed_bar2.Color := clwhite;
  if (ed_bar3.Text <> ed_bar1.Text) and (ed_bar3.Text <> ed_bar2.Text) then
    if ed_bar3.Color <> clblue then
      ed_bar3.Color := clwhite;

  if ed_bar1.Text = '' then
    ed_bar1.Color := clwhite;
  if ed_bar2.Text = '' then
    ed_bar2.Color := clwhite;
  if ed_bar3.Text = '' then
    ed_bar3.Color := clwhite;
end;

procedure TF_barang_det.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  F_barang_det := nil;
end;
...
begin
  ...
  fungsi.SQLExec(dm.Q_temp, 'select * from tb_jenis where kd_jenis="' + kd_jenis
    + '"', true);
  ...
  fungsi.SQLExec(dm.Q_temp, 'select * from tb_merk where kd_merk="' + kd_merk +
    '"', true);
  ...
  fungsi.SQLExec(dm.Q_temp, 'select * from tb_satuan where kd_satuan="' + kd_sat
    [1] + '"', true);
...
begin
  ...
  begin
    fungsi.SQLExec(dm.Q_temp,
      'select kd_barang from tb_barang where kd_barang="' + ed_pid.Text +
