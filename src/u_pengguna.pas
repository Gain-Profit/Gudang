...
begin
  ...
  if FileExists(dm.AppPath + 'image/' + ed_kode.Text + '.jpg') then
    gambar.Picture.LoadFromFile(dm.AppPath + 'image/' + ed_kode.Text + '.jpg')
  else
    gambar.Clear;

  fungsi.SQLExec(dm.Q_temp, 'select * from tb_user_company where ' + 'kd_user="'
    + kdUser + '" and kd_perusahaan="' + dm.kd_perusahaan + '"', true);
  ...
  try
    ...
    begin
      ...
      fungsi.SQLExec(dm.Q_Exe,
        'insert into tb_user (kd_user,n_user,password,`update`)values ("' +
    ...
    begin
      fungsi.SQLExec(dm.Q_Exe, 'update tb_user set `update`="' + formatdatetime('yyyy-MM-dd',
        date()) + '",n_user="' + ed_nama.Text + '",kd_user="' + ed_kode.Text +
        '" where kd_user="' + kdUser + '"', false);
      updateprivasi;
    end;
...
begin
  ...
  fungsi.SQLExec(dm.Q_Exe, 'update tb_user set `update`="' + formatdatetime('yyyy-MM-dd',
    date()) + '",password=md5("' + ed_pass.Text + '") where kd_user="' + ed_kode.Text
    + '"', false);
