...
procedure Tf_Barcode.ed_PIDKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key= vk_f2 then sb_cari.Click;

  if key=vk_return then
  begin
    if ed_PID.Text ='' then Exit;
    
    PeekMessage(Mgs, 0, WM_CHAR, WM_CHAR, PM_REMOVE );

    fungsi.SQLExec(dm.Q_temp,' SELECT tb_barang.kd_barang, tb_barang.n_barang, '+
...
begin
  ...
  laporan.LoadFromFile(dm.WPath + '\laporan\gp_barcode.fr3');
  laporan.ShowReport;
end;
...
begin
  ...
  with F_cari do
  try
    _SQLi:= 'select kd_barang, n_barang from tb_barang where kd_perusahaan="'+kd_perusahaan+'"';
    tblcap[0]:= 'PID';
...
procedure Tf_Barcode.laporanGetValue(const VarName: String;
  var Value: Variant);
begin
