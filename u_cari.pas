...
uses u_dm, u_barang_det, u_jenis, u_golongan, u_satuan, u_kategori, u_merk,
  ...
  u_pelanggan, u_emp, u_utama, u_RO, U_Login, U_kirim, u_lap,
...
begin
  case CariT of
      1 : //jenis barang_det
      begin
      fungsi.SQLExec(dm.Q_Exe,'delete from tb_jenis where kd_jenis="'+q_cari.fieldbyname('kd_jenis').AsString+'"',false);
      end;
      2 : //golongan barang_det
      begin
      fungsi.SQLExec(dm.Q_Exe,'delete from tb_golongan where kd_golbrg= "'+q_cari.fieldbyname('kd_golbrg').AsString+'" and kd_jenis="'+q_cari.fieldbyname('kd_jenis').AsString+'"',false);
      end;
      3 : //merk barang_det
      begin
      fungsi.SQLExec(dm.Q_Exe,'delete from tb_merk where kd_merk="'+q_cari.fieldbyname('kd_merk').AsString+'"',false);
      end;
      4 : //kategori barang_det
      begin
      fungsi.SQLExec(dm.Q_Exe,'delete from tb_kategori where tag="'+q_cari.fieldbyname('tag').AsString+'"',false);
      end;
      8 : //perusahaan
      begin
      fungsi.SQLExec(dm.Q_Exe,'delete from tb_company where kd_perusahaan="'+q_cari.fieldbyname('kd_perusahaan').AsString+'"',false);
      end;
      9 : //supplier
      begin
        if q_cari.fieldbyname('kode').AsString='SU-0001' then
      ...
      fungsi.SQLExec(dm.Q_Exe,'delete from tb_supp where kode="'+q_cari.fieldbyname('kode').AsString+'"',false);
      end;
      11 : //pelanggan
      begin
        if q_cari.fieldbyname('kd_pelanggan').AsString='CU-0001' then
      ...
      //satuan
      begin
      fungsi.SQLExec(dm.Q_Exe,'delete from tb_satuan where kd_satuan="'+q_cari.fieldbyname('kd_satuan').AsString+'"',false);
      end;
   end;
end;
