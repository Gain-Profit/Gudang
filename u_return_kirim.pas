...
uses
  ...
  sLabel, Mask, sCustomComboEdit, sCurrEdit, sCurrencyEdit,
  sMemo, cxGridLevel, cxGridCustomTableView, cxGridTableView, cxClasses,uTerbilang,
  cxControls, cxGridCustomView, cxGrid, sPanel,UFungsi, cxFilter, cxData,
  sMaskEdit;
...
begin
  ...
  dm.kd_perusahaan+'" and date(simpan_pada)=date(now())',true);
...
dm.kd_perusahaan+'" and kd_return_kirim="'+ed_no_faktur.Text+'"',true);
...
begin
  ...
  +'","'+formatdatetime('yyyy-MM-dd',ed_tgl.Date)+'","'+dm.kd_perusahaan+'",0,"'+ed_nilai_faktur.Text
